...
function playSample(instrument, note, destination, delaySeconds = 0) {
  getSample(instrument, note).then(({audioBuffer, distance}) => {
    ...
    var audio = new Audio();
    audio.src = bufferSource.start(audioContext.currentTime + delaySeconds);
    audio.controls = true;
    audio.loop = true;
    audio.autoplay = true;
...
fetchSample('AirportTerminal.wav').then(convolverBuffer => {
  $(".update-instrument1").click(function () {
    ...
    startLoop(selectInstrument1, noteValue, convolver, loopNum, 0);

    document.getElementById("display").innerHTML = noteValue.toString();
